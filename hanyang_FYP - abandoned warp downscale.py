...
def charRec2(img, text_recs,path, adjust=False):
    ...
    for index, rec in enumerate(text_recs):
        ...
        partImg = cv2.resize(partImg, (int(Width*3), int(Height*3)))
...
def match_outlines(orig_image, skewed_image, ii=3):
    orig_image = np.array(orig_image)
    skewed_image_ori = np.array(skewed_image).copy()
    skewed_image = np.array(skewed_image)

    #Experimental Scale Down Image before calc Homopathy, then warp Perspective after scale back up
    # scale = 2
    # Height, Width = skewed_image.shape[:2]
    # # orig_image = np.array( cv2.resize(orig_image, (int(Width / scale) ,int(Height/ scale))) )
    # skewed_image = np.array( cv2.resize(skewed_image, (int(Width / scale) ,int(Height/ scale))) )

   
    # edges = cv2.Canny(skewed_image,200,300)
    # edges2 = cv2.Canny(orig_image,200,300)
    # plt.subplot(121),plt.imshow(skewed_image,cmap = 'gray')
    ...
    if ii == 1:
        ...
        # surf = cv2.AKAZE_create(nOctaves=2,nOctaveLayers=2,descriptor_size=61) 
        surf = cv2.AKAZE_create(nOctaves=5,nOctaveLayers=4,descriptor_size=61) #nOctaves=4, nOctaveLayers=4, descriptor_size=61
        FLANN_INDEX_LSH = 6
        ...
        # lower 7 complexity, works #nlevels=7, scaleFactor=1.5
    ...
    search_params = dict(checks=5) #50)
    ...
    if len(good) > MIN_MATCH_COUNT:
        ...
        # M[0][2] = M[0][2] * scale*1
        # M[1][2] = M[1][2] * scale*1
        # M[2][0] = M[2][0] * (1/(scale*1))
        # M[2][1] = M[2][1] * (1/(scale*1))
        ...
        #deskew image
        im_out = cv2.warpPerspective(skewed_image_ori, np.linalg.inv(M),
            (int(orig_image.shape[1]), int(orig_image.shape[0]))) #+200 #orig_image
        return im_out, nn

    else:
        print("MAP: Not  enough  matches are found   -   %d/%d"
        			  % (len(good), MIN_MATCH_COUNT))
        return skewed_image_ori , nn
...
def start(imgPath):
    ...
    rHelperFunc.newHeight = resolution
    rHelperFunc.originalHeight = 530
    rHelper = rHelperFunc()
    text_recs, img_framed, image = get_det_boxes(image, resolution)
    ...
    start_time = time.time()
    name, id_num, add, info = "","","",""
    nt1, nt2 = "",""
    it1, it2 = "",""
    at1, at2 =""," "
    add1, add2 = "",""
    #print("res length =" +str(len(res)))
    #print("coord length =" +str(len(coord_all)))
    
    #Dont OCR First 2 results
    for i in range(len(res))[2:]:
        #print(res[i])
        imgP = cv2.imread(res[i])
        #Resize for better OCR Results
        #Height, Width = imgP.shape[:2]
        #imgP = cv2.resize(imgP, (int(Width*3), int(Height*3)))
        coord = coord_all[i]        
        text2 = single_pic_proc2(res[i])
        print(coord,text2)
        
        #Exact pixel location it seems, just use MsPaint to find out what area u need and bamm
        #MyKad
        if coord[1] >= rHelper(360) and coord[3] <= rHelper(430):    
            it1 = it1 + "^" + text2
        else:
            imgP = cv2.imread(res[i])  
...
imgP =img = cv2.imread(newImgPath)
...
#couldnt get SURF on latest OpenCV 4.5?
...
newPath = dir_path +"/output/pr_ready.jpg"


#newPath = directoryName+"//croppedIC.jpg"
if flag1 > flag2:		
    print("flag1")	
    cv2.imwrite(newPath, image1)	
else:
    #imgPath = directoryName+"//croppedIC.jpg"			
    print("flag2")	
    cv2.imwrite(newPath, image1)
