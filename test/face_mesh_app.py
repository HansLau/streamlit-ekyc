...
app_mode = st.sidebar.selectbox('Choose e-KYC Process',
['e-KYC Introduction and Explanation','1. IC Card and Face Verification (IMAGES)','2. Mouth Detection Anti-Spoofing (VIDEO)']
)
...
if app_mode =='e-KYC Introduction and Explanation':
...
elif app_mode =='1. IC Card and Face Verification (IMAGES)':
    ...
    with col2:
        ...
        verify = st.button('Verify IC with Selfie Face')
        if verify:
...
elif app_mode =='2. Mouth Detection Anti-Spoofing (VIDEO)':
    ...
    if not use_webcam:
        video_file_buffer = st.file_uploader("Upload a video for Mouth Detection Anti-Spoofing", type=[ "mp4", "mov",'avi','asf', 'm4v' ])
