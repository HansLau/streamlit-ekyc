...
prob_thresh = 0.5
height = 380 #560 default
gpu = True
if not torch.cuda.is_available():
    gpu = False
device = torch.device('cuda:0' if gpu else 'cpu')
weights = os.path.join(config.checkpoints_dir, r'CTPN.pth')
...
def get_det_boxes(image, height=height, display = True, expand = True):
    ...
    with torch.no_grad():
        ...
        keep = nms(nmsbox, 0.2) #original 0.3
        # print(keep)
        ...
        # expand text
        if expand:
            for idx in range(len(text)): #35
                text[idx][0] = max(text[idx][0] - 45, 0)
                text[idx][2] = min(text[idx][2] + 45, w - 1)
                text[idx][4] = max(text[idx][4] - 45, 0)
                text[idx][6] = min(text[idx][6] + 45, w - 1)
                
                text[idx][5] = min(text[idx][5] + 2, h - 1) #2
                text[idx][7] = min(text[idx][7] + 2, h - 1) #2
